...
| `GET /object-search-metadata/rows/{row_index}/neighbor-projections?count=N` | Lift the proposal centre through its stored depth and select N nearby manifest poses (1–12) with a 0.5 m baseline when possible. Returns the projected centre/extent, an explainable geometric confidence (source distance, viewpoint angle, apparent size), a target-depth visibility confidence, and a COLMAP-style feature score. The feature score runs SIFT on the centred proposal region of source/target 2×-FOV renders, applies L2 2-NN ratio matching, verifies a homography with RANSAC, and combines inlier ratio with verified support; missing OpenCV/SIFT reports `unavailable` without failing the projection. Target depth near the expected range is `clear`; nearer depth is `occluded`; farther depth is `depth_mismatch`; missing/undecodable depth stays `unknown` without failing the geometric result. |
